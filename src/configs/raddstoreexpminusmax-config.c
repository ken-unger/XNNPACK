...
static void init_f16_raddstoreexpminusmax_config(void) {
    ...
    #endif
  #elif XNN_ENABLE_RISCV_FP16_VECTOR && XNN_ARCH_RISCV
    const struct xnn_hardware_config* hardware_config = xnn_init_hardware_config();
    assert(hardware_config != NULL);
    (void) hardware_config;  // May be unused.
    if (hardware_config->arch_flags & xnn_arch_riscv_vector_fp16_arith) {
      f16_raddstoreexpminusmax_config.ukernel = XNN_INIT_RADDSTOREEXPMINUSMAX_UKERNEL(xnn_f16_raddstoreexpminusmax_ukernel__rvvfp16arith_rr2_p2_u8v);
    }
  #endif
}
