    ...
    const void* params) XNN_OOB_READS
{
  ...
  do {
    ...
    const vfloat16m8_t vx = __riscv_vfsub(vi, *max, vl);

    vfloat16m8_t vn = __riscv_vfmv_v_f_f16m8(vmagic_bias, vl);
    vn = __riscv_vfmacc(vn, vlog2e, vx, vl);

    const vfloat16m8_t vs = __riscv_vreinterpret_v_i16m8_f16m8(__riscv_vsll(__riscv_vreinterpret_v_f16m8_i16m8(vn), 10, vl));
    ...
    const vbool2_t vmask = __riscv_vmflt(vx, vdenorm_cutoff, vl);
    vf = __riscv_vfmerge(vf, 0.0f, vmask, vl);
    //vf = __riscv_vreinterpret_v_u16m8_f16m8(__riscv_vand(vmask, __riscv_vreinterpret_v_f16m8_u16m8(vf), UINT16_C(0x0000), vl));
