...
#include "test/raddstoreexpminusmax-microkernel-tester.h"
...
#if XNN_ENABLE_RISCV_FP16_VECTOR && XNN_ARCH_RISCV
  TEST(F16_RADDSTOREEXPMINUSMAX__RVVFP16ARITH_RR2_P2_U8V, elements_eq_8v) {
    TEST_REQUIRES_ARCH_FLAGS(xnn_arch_riscv_vector_fp16_arith);
    RAddStoreExpMinusMaxMicrokernelTester()
      .elements(8 * xnn_init_hardware_config()->vlenb / sizeof(uint16_t))
      .Test(xnn_f16_raddstoreexpminusmax_ukernel__rvvfp16arith_rr2_p2_u8v, nullptr);
  }

  TEST(F16_RADDSTOREEXPMINUSMAX__RVVFP16ARITH_RR2_P2_U8V, elements_div_8v) {
    TEST_REQUIRES_ARCH_FLAGS(xnn_arch_riscv_vector_fp16_arith);
    for (size_t elements = 16 * xnn_init_hardware_config()->vlenb / sizeof(uint16_t);
                elements < 80 * xnn_init_hardware_config()->vlenb / sizeof(uint16_t);
                elements += 8 * xnn_init_hardware_config()->vlenb / sizeof(uint16_t)) {
      RAddStoreExpMinusMaxMicrokernelTester()
        .elements(elements)
        .Test(xnn_f16_raddstoreexpminusmax_ukernel__rvvfp16arith_rr2_p2_u8v, nullptr);
  ...
  TEST(F16_RADDSTOREEXPMINUSMAX__RVVFP16ARITH_RR2_P2_U8V, elements_lt_8v) {
    TEST_REQUIRES_ARCH_FLAGS(xnn_arch_riscv_vector_fp16_arith);
    for (size_t elements = 1;
                elements < 8 * xnn_init_hardware_config()->vlenb / sizeof(uint16_t);
                elements++) {
  ...
  TEST(F16_RADDSTOREEXPMINUSMAX__RVVFP16ARITH_RR2_P2_U8V, elements_gt_8v) {
    TEST_REQUIRES_ARCH_FLAGS(xnn_arch_riscv_vector_fp16_arith);
    for (size_t elements = 8 * xnn_init_hardware_config()->vlenb / sizeof(uint16_t) + 1;
                elements < 16 * xnn_init_hardware_config()->vlenb / sizeof(uint16_t);
                elements += 16) {
      RAddStoreExpMinusMaxMicrokernelTester()
        .elements(elements)
        .Test(xnn_f16_raddstoreexpminusmax_ukernel__rvvfp16arith_rr2_p2_u8v, nullptr);
